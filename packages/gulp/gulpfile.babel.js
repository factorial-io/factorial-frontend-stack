...
function compileJS(flag) {
  const bundler = watchify(
    browserify("./lib/index.js", { debug: true }).transform(babel)
  );
...
gulp.task("test:browser", () =>
  gulp.src("test/behavior/behavior.html").pipe(mochaPhantomJS())
);

gulp.task("build:test", () => {
  gulp
    .src("test/visual/visual.html")
    .pipe(plumber())
...
gulp.task("watch:test", () => {
  gulp.watch(["test/visual/**/*.*"], ["build:test"]);
});

gulp.task("default", ["watch:css", "watch:js", "watch:test", "server"]);
